...
if TYPE_CHECKING:
    ...
    # Emit per-request [KV] ALLOCATED/FREED traces (EngineCore stdout).
    VLLM_KV_CACHE_TRACE: bool = False
    # Phase label in [KV] traces: "auto" (PREFILL/DECODE/VERIFY) or a fixed tag.
    VLLM_KV_CACHE_TRACE_PHASE: str = "auto"
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # Print per-request [KV] ALLOCATED / FREED lines (very verbose).
    "VLLM_KV_CACHE_TRACE": lambda: bool(
        int(os.getenv("VLLM_KV_CACHE_TRACE", "0"))
    ),
    # Phase tag in [KV] traces. "auto" picks PREFILL / DECODE / VERIFY per call;
    # any other value is printed as a fixed label.
    "VLLM_KV_CACHE_TRACE_PHASE": lambda: os.getenv(
        "VLLM_KV_CACHE_TRACE_PHASE", "auto"
    ),
